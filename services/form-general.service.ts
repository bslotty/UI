...
export class FormGeneralService {
  ...
  initControls(form: UntypedFormGroup, section: FormTypes): FormInputField[] {
    switch (section) {
      case FormTypes.search:
        return [
          new FormInputField('Search Term', FormInputTypes.text).setControl(
            form.get('term') as FormControl,
          ),
        ];

      case FormTypes.dateRange:
        return [
          new FormInputField('Start', FormInputTypes.date).setControl(
            form.get('start') as FormControl,
          ),
          new FormInputField('End', FormInputTypes.date).setControl(
            form.get('end') as FormControl,
          ),
        ];

      case FormTypes.preset:
        return [
          new FormInputField('Preset', FormInputTypes.autocomplete).setControl(
            form.get('preset') as FormControl,
          ),
        ];
